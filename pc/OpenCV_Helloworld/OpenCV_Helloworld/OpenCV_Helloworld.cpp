...
int main() {
	...
	//capture = cvCreateFileCapture("H:/DCIM/100DSCIM/PICT0003.AVI"); // chemin pour un fichier
	...
	cvNamedWindow("GeckoGeek Color Rectification",CV_WINDOW_NORMAL);
   	cvNamedWindow("GeckoGeek Mask",CV_WINDOW_NORMAL);
	cvResizeWindow("GeckoGeek Mask",600,500);
	cvResizeWindow("GeckoGeek Color Rectification",600,500);
	cvResizeWindow("GeckoGeek Color Tracking",600,500);
	cvMoveWindow("GeckoGeek Color Tracking", 10, 10);
	cvMoveWindow("GeckoGeek Color Rectification", 10, 550);
	cvMoveWindow("GeckoGeek Mask", 650, 10);
...
/*int _tmain(int argc, _TCHAR* argv[])
{
	...
	char FPS[50] = {"FPS : 00000"};
 
	// Ouvrir le flux vidéo
	//capture = cvCreateFileCapture("J:/DCIM/100DSCIM/PICT0001.AVI"); // chemin pour un fichier
	capture = cvCreateCameraCapture(CV_CAP_ANY);
	...
	while(key != 'q' && key != 'Q') {
	    ...
	    image = cvQueryFrame(capture);
		sprintf(FPS,"FPS : %i",(int)(1/((float)(clock()-last)/CLOCKS_PER_SEC)) );
		
		last = clock();
	
   
		cvPutText(image, FPS, cvPoint(0,30), &font, cvScalar(255, 255, 255, 0));
